...
int main() {
    ...
    glm::vec3 cameraDirection = glm::normalize(cameraPosition - cameraTarget); //actualliy points in the inverse direction to teh direction
    //world up
    glm::vec3 up = glm::vec3(0.0f, 1.0f, 0.0f);
    //to obtain the right vector, we find a vector perpendicular to teh direction and the world up vector (to do so, we can use the crossproduct of them)
    glm::vec3 cameraRight = glm::normalize(glm::cross(up, cameraDirection));
    glm::vec3 cameraUp = glm::cross(cameraRight, cameraDirection); //then repeat to obtain the camera up vector (no need to normalize since the inputs are orthonormal)

    //creating teh view matrix
    glm::mat4 view;
